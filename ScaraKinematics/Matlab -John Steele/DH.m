...
if (s(1) <= 1 || s(2) ~= 4)
    error('Paramater Array has incorrect dimensions.');
end

n = s(1);

if (jnt < 1 || jnt > n)
    error('User specified nonexistant joint.');
end
...
for i = 1:jnt
    ...
    dh_i = [    cosd(t)             -sind(t)*cosd(p)    sind(t)*sind(p)     a*cosd(t)           ;
                sind(t)             cosd(t)*cosd(p)     -cosd(t)*sind(p)    a*sind(t)           ;
                0                   sind(p)             cosd(p)             d                   ;
                0                   0                   0                   1                       ];
