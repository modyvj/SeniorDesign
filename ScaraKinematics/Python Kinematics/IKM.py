...
def IKM(arms=None, pos=None):
    ...
    if (max(s_arms) != 5 or min(s_arms) != 1):
        error(mstring('Link Array has incorrect dimensions.'))
    elif (max(s_end) != 5 or min(s_end) != 1):
        error(mstring('Target Array has incorrect dimensions.'))
        end
